...
#
#
class RenewalPassportSerializer(serializers.ModelSerializer):
    class Meta:
        model = RenewalPassportApplication
    ...
    def validate_old_passport_pdf(self, value):
        if value:
            if not value.name.endswith(".pdf"):
                raise ValidationError("Only PDF files are allowed for old passport pdf")
            if value.size > 1024 * 1024:  # 1 MB
                raise ValidationError(
                    "Old passport pdf file size too large. Max size allowed is 1 MB"
                )
        return value
